...
<div>
	<div class="flex flex-1 flex-col">
		<div class="card mb-2 mb-2 p-3 text-base dark:variant-soft-tertiary">
			<p class="break-all">
				<span class="font-semibold">{$i18n.t('roleName')}:</span>
				{getCellName(cellInfo)}
			</p>
			{#if cellId}
				<p class="break-all">
					<span class="font-semibold">{$i18n.t('dnaHash')}:</span>
			...
			<div class="flex flex-row items-center">
				...
				{#if showNetworkSeed}
					<svg
						...
						viewBox="0 0 26 18"
						fill="none"
						style="cursor: pointer; fill: white; opacity: 0.8"
						version="1.1"
						xmlns="http://www.w3.org/2000/svg"
					>
						<title>{capitalizeFirstLetter($i18n.t('hide')) + ' ' + $i18n.t('networkSeed')}</title>
						<path
				...
				{:else}
					<svg
						...
						xmlns="http://www.w3.org/2000/svg"
					>
						<title>{capitalizeFirstLetter($i18n.t('show')) + ' ' + $i18n.t('networkSeed')}</title>
						<path
